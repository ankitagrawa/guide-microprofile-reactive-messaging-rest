...
The `inventory` microservice uses an incoming connector to receive messages through the [hotspot=systemLoad file=1]`systemLoad` and [hotspot=propertyMessage file=1]`propertyMessage` channel. The messages were published by the `system` microservice to the [hotspot=topic1 file=1]`systemLoadTopic` and [hotspot=topic3 file=1]`propertyMessageTopic` in the Kafka message broker. The [hotspot=deserializer1 hotspot=deserializer3 file=1]`key.deserializer` and [hotspot=deserializerVal1 hotspot=deserializerVal3 file=1]`value.deserializer` properties define how to deserialize the messages. The `SystemLoadDeserializer` and `PropertyMessageDeserializer` class implements the logic for turning JSON into a `SystemLoad` and `PropertyMessage` object and is configured as the [hotspot=deserializerVal1 hotspot=deserializerVal3 file=1]`value.deserializer`. The [hotspot=group1 file=1]`group.id` property defines a unique name for the consumer group.
