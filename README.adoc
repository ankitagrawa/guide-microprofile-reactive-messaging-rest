...
:page-seo-title: Creating asynchronous Java rest microservices using MicroProfile Reactive Messaging 
...
= Creating RESTful reactive Java microservices
...
//If you want to learn more about RESTful web services and how to build them, see `Creating a RESTful web service` for details about how to build the system service. 

== What you'll learn

You will learn how to trigger RESTful reactive microservices to send requests to other microservices, and asynchronously receive and process the responses. Currently MicroProfile Reactive Messaging implementation works on continuous event streams. i.e. Producer continuously produces events and Consumer consumes those events. This guide helps you to write microservices so that the Producer events would be created when a user hits the endpoint.

If you want to learn more about basic MicroProfile Profile Reactive Messaging basics, see https://openliberty.io/guides/microprofile-reactive-messaging.html[Creating reactive Java microservices^]. It's strongly recommended to go through the basic guide before you go further.
...
The application in this guide consists of two microservices, `system` and `inventory`. Every 15 seconds, the `system` microservice calculates and publishes events that contains its current average system load and current and maximum JVM heap memory usage. The `inventory` microservice subscribes to that information so that it can keep an updated list of all the systems and their current system loads and JVM heap memory usage properties. The `invetory` microservice also subscribes to a POST request response that queries a specific system property on the `system` microservice. The current inventory of systems can be accessed via the `/systems` REST endpoint. You'll create the `system` and `inventory` microservices using MicroProfile Reactive Messaging.
...
The `system` microservice is the producer of the messages that are published to the Kafka messaging system as a stream of events. Every 15 seconds, the `system` microservice publishes events that contains its calculation of the average system load (its CPU usage) for the last minute and current and maximum JVM heap memory usage.
It also processes the specific system property request from the `inventory` microservice and publishes it to the Kafka messaging system.
...
The `SystemService` class contains multiple `Publisher` methods [hotspot=sendSystemLoad file=0]`sendSystemLoad()`, which calculates and returns the average system load and [hotspot=sendMemoryUsage file=0]`sendMemoryUsage()`, which calculates and return used and max JVM memory. The [hotspot=publishSystemLoad file=0]`@Outgoing` annotation on the [hotspot=sendSystemLoad file=0]`sendSystemLoad()` and [hotspot=publishMemoryUsage file=0]`sendMemoryUsage()` method indicates that the method publishes its calculation as a message on a topic in the Kafka messaging system. The [hotspot=flowableInterval file=0]`Flowable.interval()` method from `rxJava` is used to set the frequency of how often the system service publishes the calculation to the event stream.

The `system` microservice also contains a method called [hotspot=sendPropertyDetails file=0]`sendProperty` that receives a system property name from the `inventory` microservice and calculates the requested property on the go and publishes it back to the Kafka. The [hotspot=sendPropertyDetails file=0]`sendProperty` method acts as a processor in this scenario.

The messages are transported between the service and the Kafka messaging system through the channels called [hotspot=systemLoad file=1]`systemLoad`, [hotspot=memoryStatus file=1]`memoryStatus` and [hotspot=setProperty file=1]`setProperty`.

== Creating the inventory microservice

The `inventory` microservice records in its inventory the average system load, used and max JVM memory information and the requested system property, that it received from potentially multiple instances of the `system` service.
...
The `inventory` microservice receives the message from the `system` microservice over the [hotspot=systemLoad file=0]`@Incoming("systemLoad")`, [hotspot=memoryStatus file=0]`@Incoming("memoryStatus")` and [hotspot=propertyMessage file=0]`@Incoming("propertyMessage")` channel. The properties of this channel are defined in the [hotspot=systemLoad file=1]`microprofile-config.properties` file. The `inventory` microservice is also a RESTful service that is served at the [hotspot=inventoryEndPoint file=0]`/inventory` endpoint.

The `InventoryResource` class contains an oveloaded method called [hotspot=updateStatus file=0]`updateStatus()`, which receives the message that contains the average system load, used and max JVM memory information and updates its existing inventory of systems and their properties. The [hotspot=systemLoad file=0]`@Incoming("systemLoad")` and [hotspot=memoryStatus file=0]`@Incoming("memoryStatus")` annotations on the [hotspot=updateStatus file=0]`updateStatus()` method indicates that the method retrieves the average system load and used and max JVM memory information by connecting to the channel called [hotspot=systemLoad file=0]`systemLoad` and [hotspot=memoryStatus file=0]`memoryStatus` respectively.

The `inventory` microservice creates a system property request when a property name is sent in a POST request to the `/systems/{propertyName}` endpoint. The getSystemProperty() method receives a property name and adds it to the FlowableEmitter. The Flowable.create() method from rxJava is used to create a Publisher and an associated emitter. The property name sent to the emitter will be sent to the Publisher. The Publisher is returned from the @Outgoing("propertyName") channel, which is configured in the microprofile-config.properties propertyName stream and MicroProfile Reactive Messaging takes care of assigning the publisher to the channel.
...
The [hotspot=kafkaConfig file=0]`mp.messaging.connector.liberty-kafka.bootstrap.servers` property configures the hostname and port for connecting to the Kafka server. The `system` microservice uses an outgoing connector to send messages through the [hotspot=systemLoad file=0]`systemLoad`, [hotspot=memoryStatus file=0]`memoryStatus` and [hotspot=setProperty file=0]`setProperty` channel to the [hotspot=topic1 file=0]`systemLoadTopic`, [hotspot=topic2 file=0]`memoryStatusTopic` and [hotspot=topic3 file=0]`propertyMessageTopic` topic in the Kafka message broker so that the `inventory` microservices can consume the messages. The [hotspot=serializer1 hotspot=serializer2 hotspot=serializer3 file=0]`key.serializer` and [hotspot=serializerVal1 hotspot=serializerVal2 hotspot=serializerVal3 file=0]`value.serializer` properties characterize how to serialize the messages. The `SystemLoadSerializer` and `MemoryStatusSerializer` class implements the logic for turning a `SystemLoad` and `MemoryStatus` object into JSON and is configured as the [hotspot=serializerVal1 hotspot=serializerVal2 hotspot=serializerVal3 file=0]`value.serializer`.
...
The `inventory` microservice uses an incoming connector to receive messages through the [hotspot=systemLoad file=1]`systemLoad`, [hotspot=memoryStatus file=1]`memoryStatus` and [hotspot=propertyMessage file=1]`propertyMessage` channel. The messages were published by the `system` microservice to the [hotspot=topic1 file=1]`systemLoadTopic`,[hotspot=topic2 file=1]`memoryStatusTopic` and [hotspot=topic3 file=1]`propertyMessageTopic` in the Kafka message broker. The [hotspot=deserializer1 hotspot=deserializer2 hotspot=deserializer3 file=1]`key.deserializer` and [hotspot=deserializerVal1 hotspot=deserializerVal2 hotspot=deserializerVal3 file=1]`value.deserializer` properties define how to deserialize the messages. The `SystemLoadDeserializer`, `MemoryStatusDeserializer` and `PropertyMessageDeserializer` class implements the logic for turning JSON into a `SystemLoad`, `MemoryStatus` and `PropertyMessage` object and is configured as the [hotspot=deserializerVal1 hotspot=deserializerVal2 hotspot=deserializerVal3 file=1]`value.deserializer`. The [hotspot=group1 file=1]`group.id` property defines a unique name for the consumer group. A consumer group is a collection of consumers who share a common identifier for the group. You can also view a consumer group as the various machines that ingest from the Kafka topics. All of these properties are required by the https://kafka.apache.org/documentation/#producerconfigs[Apache Kafka Producer Configs^] and https://kafka.apache.org/documentation/#consumerconfigs[Apache Kafka Consumer Configs^].
...
Go to the http://localhost:9085/inventory/systems[^] URL to access the inventory microservice. You see the CPU `systemLoad`, JVM `memoryUsed` and `memoryMax` property for all the systems:
...
----
{
   ...
   "memoryUsed":50582400,
   "memoryMax":1032257536,
   "systemLoad":1.44
}
----

You can revisit the http://localhost:9085/inventory/systems[^] URL after a while, and you will notice the CPU `systemLoad`, JVM `memoryUsed` property for the systems changed.

You can use the `\http://localhost:9085/inventory/systems/{hostname}` URL to see the CPU `systemLoad`, JVM `memoryUsed` and `memoryMax` property for one particular system.

In the following example, the `30bec2b63a96` value is the `hostname`. If you go to the `\http://localhost:9085/inventory/systems/30bec2b63a96` URL, you can see the properties only for the `30bec2b63a96` `hostname`:
...
For ex. : The `\http://localhost:9085/inventory/systems/property/os.name` will give you the os.name value for your system.
...
into the URL bar. Click the blue `Send` button to make the request.
--
