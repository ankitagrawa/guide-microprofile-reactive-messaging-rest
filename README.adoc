...
The `SystemService` class contains multiple `Publisher` methods [hotspot=sendSystemLoad file=0]`sendSystemLoad()`, which calculates and returns the average system load and [hotspot=sendMemoryUsage file=0]`sendMemoryUsage()`, which calculates and return the used and max JVM memory. The [hotspot=publishSystemLoad hotspot=publishMemoryUsage file=0]`@Outgoing` annotation on the [hotspot=sendSystemLoad file=0]`sendSystemLoad()` and the [hotspot=sendMemoryUsage file=0]`sendMemoryUsage()` method indicates that the method publishes its calculation as a message on a topic in the Kafka messaging system. The [hotspot=flowableInterval hotspot=flowableInterval1 file=0]`Flowable.interval()` method from `rxJava` is used to set the frequency of how often the system service publishes the calculation to the event stream.
...
The `inventory` microservice receives the message from the `system` microservice over the [hotspot=systemLoad file=0]`@Incoming("systemLoad")`, [hotspot=memoryStatus file=0]`@Incoming("memoryStatus")` and [hotspot=propertyMessage file=0]`@Incoming("propertyMessage")` channel. The properties of this channel are defined in the [hotspot=systemLoad hotspot=memoryStatus hotspot=propertyMessage file=1]`microprofile-config.properties` file. The `inventory` microservice is also a RESTful service that is served at the [hotspot=inventoryEndPoint file=0]`/inventory` endpoint.

The `InventoryResource` class contains an overloaded method called [hotspot=updateStatus hotspot=updateStatus1 file=0]`updateStatus()`, which receives the messages that contains the average system load, the used and max JVM memory information and updates its existing inventory of systems and their properties. The [hotspot=systemLoad file=0]`@Incoming("systemLoad")` and [hotspot=memoryStatus file=0]`@Incoming("memoryStatus")` annotations on the [hotspot=updateStatus hotspot=updateStatus1 file=0]`updateStatus()` method indicates that the method retrieves the average system load and the used and max JVM memory information by connecting to the channel called [hotspot=systemLoad file=0]`systemLoad` and [hotspot=memoryStatus file=0]`memoryStatus` respectively.
...
Go to the http://localhost:9085/inventory/systems[^] URL to access the inventory microservice. You see the CPU `systemLoad`, JVM `memoryUsed` and `memoryMax` properties for all the systems:
...
You can revisit the http://localhost:9085/inventory/systems[^] URL after a while, and you will notice the CPU `systemLoad`, JVM `memoryUsed` properties for the systems changed.

You can use the `\http://localhost:9085/inventory/systems/{hostname}` URL to see the CPU `systemLoad`, JVM `memoryUsed` and `memoryMax` properties for one particular system.
